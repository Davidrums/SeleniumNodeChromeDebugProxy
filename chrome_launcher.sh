...
exec -a "$0" "$HERE/chrome" --no-sandbox --proxy-server="$PROXY_SERVER" "$PROFILE_DIRECTORY_FLAG" \
  "$@"
exec -a "$0" /etc/alternatives/google-chrome --no-sandbox --proxy-server="$PROXY_SERVER" "$@"